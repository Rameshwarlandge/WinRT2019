...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	WCHAR buffer[255];
	HINSTANCE hDll;
	typedef INT(*fp_ExplicitLink)(INT);
	fp_ExplicitLink  fp;
	switch (iMsg)
	{
	case WM_CREATE:
		hDll = LoadLibrary(TEXT("DEF.dll"));
		fp = (fp_ExplicitLink)GetProcAddress(hDll, "MY_MakeSquare");
